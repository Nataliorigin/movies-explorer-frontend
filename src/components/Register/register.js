        ...
        <section className="register">
          ...
          <form className="register__form" onSubmit={handleRegister}>
            <label className='register__container'>
              ...
              <input type='text' name='name' placeholder='Имя' className='register__input' value={values.name || ""}
            ...
            <label className='register__container'>
              ...
              <input type='email' name='email' placeholder='Email' className='register__input'
                     value={values.email || ""} onChange={handleInputsChange}
                     minLength={5} maxLength={30} pattern="^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$" required/>
              <span className='register__error'>{errors.email}</span>
            </label>
            <label className='register__container'>
              <span className='register__input-name'>Пароль</span>
              <input type='password' name='password' placeholder='Пароль' className='register__input'
                     value={values.password || ""} onChange={handleInputsChange}
                     minLength={8} maxLength={30} required/>
              <span className='register__error'>{errors.password}</span>
            </label>
            <p
              className={`register__error-message ${errorMessage && 'register__error-message_visible'}`}>{errorMessage}</p>
            <button type="submit" className={`register__button ${!isValid && 'register__button_disabled'}`}
                    disabled={!isValid || isDisabledButton} title="Зарегистрироваться">Зарегистрироваться
            </button>
          </form>
